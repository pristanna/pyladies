...
def tah_pocitace(pole, symbol):
    ...
    ### Toto je útočná strategie počítače - klást symboly vždy vedle
    index = 0
    ...
    if symbol in pole:
        ...
        while True:
            if len(obsazena_policka_pc(pole)) > index:
                ...
                # Zkousim jestli je volna pozice vpravo od hráčova symbolu
                cislo_policka = obsazena_policka_pc(pole)[index] + 1
                if cislo_policka not in obsazena_policka(pole) and cislo_policka < 20:
                    return tah(pole, cislo_policka, symbol)

                # Zkousim jestli je volna pozice vlevo od hráčova symbolu
                cislo_policka = obsazena_policka_pc(pole)[index] - 1
                if cislo_policka not in obsazena_policka(pole) and cislo_policka < 20:
                    return tah(pole, cislo_policka, symbol)
                index += 1
